...
class Modal(object):
    ...
    def validate(self, data='dev', do_test=False):
        ...
        btm = time.time()
        result_collection = {}
        # f = open("end-to-end_Response_generation_RL.txt","w")
        # f.close()
        with torch.no_grad():
            print("starting chatbot...")
            f = open("end-to-end_Response_generation_RL.txt","a")
            f.writelines("---------------------------------------------"+"\n")
            ...
            for i in range(20):
                ...
                if cfg.use_true_curr_bspn: # generate act, response
                    ...
                    print("generated negotiation strategy: ",s2)
                    f.writelines("generated negotiation strategy:"+s2+"\n")

                    inputs['context_tensor_act'] = torch.tensor([inputs['context'][:-1] + bspn_gen + self.tokenizer.encode(s2) + self.tokenizer.encode('<sos_a>')]).to(self.device)
                    context_length = len(inputs['context_tensor_act'][0])
                ...
                pv_turn['n_strategy'] = self.tokenizer.encode(s2) #############################################
