...
class BLEUScorer(object):
    ...
    def perplexity(self,hyps,refs):
        a = hyps
        b = refs
        a = [[' '.join(i)] for i in a][0]
        b = [[' '.join(i)] for i in b][0]

        tokenizer = GPT2Tokenizer.from_pretrained(cfg.gpt_path)
        a = torch.FloatTensor(tokenizer.decode(a))
        b = torch.FloatTensor(tokenizer.decode(b))
        l = nn.CrossEntropyLoss(reduction = "sum")
        
        return pow(np.exp,l(b,a))
        


    def calculate_rouge(self,hyps,refs):
        a = hyps
        b = refs
        a = [[' '.join(i)] for i in a][0][0]
        b = [[' '.join(i)] for i in b][0][0]

        scorer = rouge_scorer.RougeScorer(['rouge1','rougeL'], use_stemmer=True)
        scores = scorer.score(str(a),str(b))
                                        
        return scores['rouge1'][2],scores['rougeL'][2]
    ...
    def score(self, parallel_corpus):
        ...
        for hyps, refs in parallel_corpus:
            ...
            print("hyps:",hyps)
            print("ref:",refs   )
            rouge_one, rougel = self.calculate_rouge(hyps,refs)
            rouge1 += rouge_one 
            rougeL += rougel

            prc_, rec_, f1_ = self.precision_recall_f1(hyps,refs)
            prc += prc_
            rec += rec_
            f1 += f1_

            #perp += self.perplexity(hyps,refs)
            counter += 1
            for hyp in hyps:
                print('hyp:',hyp)
                ...
                if len(refs[0])>=4 and len(hyps[0])>=4 :
                    for i in range(4):
                        # accumulate ngram counts
                        hypcnts = Counter(ngrams(hyp, i + 1))
                        print("hello:",ngrams(hyp,i+1))
        ...
        return bleu * 100,rouge1/counter,rougeL/counter,BLEU1/counter,BLEU2/counter,BLEU3/counter,BLEU4/counter,prc/counter,rec/counter,f1/counter
...
class MultiWozEvaluator(object):
    ...
    def validation_metric(self, data):
        bleu,rouge1,rougeL,BLEU1,BLEU2,BLEU3,BLEU4,prc,rec,f1 = self.bleu_metric(data)
        ...
        return bleu, success, match,rouge1, rougeL, BLEU1, BLEU2, BLEU3, BLEU4, prc, rec, f1

    def _get_metric_results(self, data, domain='all', file_list=None):
        metric_result = {'domain': domain}
        bleu,rouge,perp = self.bleu_metric(data, file_list)
    ...
    def bleu_metric(self, data, eval_dial_list=None):
        ...
        wrap_generated = [[gen[i]]for i in range(len(gen)) if truth[i]!='']#[[_] for _ in gen]
        wrap_truth = [[truth[i]] for i in range(len(truth)) if truth[i]!='']#[[_] for _ in truth]

        print("wrap_gen:",wrap_generated)

        print("gen:",gen)
        print("truth:",truth)
        if gen and truth:
            sc,rouge1,rougeL,BLEU1,BLEU2,BLEU3,BLEU4,prc,rec,f1 = self.bleu_scorer.score(zip(wrap_generated, wrap_truth))
        else:
            sc = 0.0
            rouge1 = 0.0
            rougeL = 0
            ...
            rec = 0
            f1 = 0
        return sc,rouge1,rougeL,BLEU1,BLEU2,BLEU3,BLEU4,prc,rec,f1
