...
class MultiWozReader(_ReaderBase):
    ...
    def get_exp_domains(self, exp_domains, all_domains_list):
        if 'common' in exp_domains:
            if 'except' in exp_domains:
                # ['except', 'hotel']
                domains = [d for d in all_domains_list if 'common' not in d and 'multi' not in d]
            else:
                # ['hotel']
                domains = ['common_single', 'common_multi']
        return domains
    ...
    def _construct_bspn_constraint(self):
        ...
        with open('CLB/clb-processed/bspn_masks.txt', 'w') as f:
    ...
    def _construct_aspn_constraint(self):
        ...
        with open('CLB/clb-processed/aspn_masks.txt', 'w') as f:
    ...
    def _load_data(self, save_temp=True):
        ...
        if save_temp: # save encoded data
            ...
            if os.path.exists(encoded_file):
                ...
                logging.info('Loading data from {}'.format(cfg.data_path+cfg.data_file))
                self.data = json.loads(
                    open(cfg.data_path+cfg.data_file, 'r', encoding='utf-8').read())#.lower())
                self.train, self.dev, self.test = [], [], []
    ...
    def _get_encoded_data(self, fn, dial):
        ...
        for idx, t in enumerate(dial['log']):  # tokenize to list of ids
            ...
            enc['bsdx'] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(
                '<sos_b> ' +
                t['cons_delex'] + ' <eos_b>'))
            enc['n_strategy'] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(
                '<sos_ns> ' +
                t['n_strategy'] + ' <eos_ns>'))
            ...
            enc['dspn'] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(
                '<sos_d> ' +
                t['turn_domain'] + ' <eos_d>'))
            enc['sentiment'] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(
                '<sos_s> ' +
                t['sentiment'] + ' <eos_s>'))
    ...
    def convert_turn_eval(self, turn, pv_turn, first_turn=False):
        ...
        if cfg.use_true_curr_bspn:
            if cfg.use_true_curr_aspn: # only predict resp
                context_list = ['user','sentiment','bspn','n_strategy','aspn']
                #context_list = ['user','bspn','db']
                # context_list = ['user','aspn'] # predict resp based on current aspn and bspn
                # predict_list = ['resp']
                prompt = '<sos_r>'
            else: # predicted aspn
                context_list = ['user','sentiment','bspn','n_strategy']
                #context_list = ['user','bspn','db']
        ...
        if first_turn:
            ...
            pv_context = pv_turn['labels'] + pv_turn['sentiment'] + pv_turn['bspn'] + pv_turn['n_strategy'] + pv_turn['aspn'] + pv_turn['resp']
            #pv_context = pv_turn['labels'] + pv_turn['bspn'] + pv_turn['db'] + pv_turn['resp']
            # e43 with BABAU
    ...
    def convert_batch_session(self, dial_batch):
        ...
        concat [U_0, S_0, B_0, A_0, R_0, ... , U_n, S_n, B_n, A_n, R_n]

        try: [user, sentiment, bspn, aspn, resp]
        or
        try: [user, sentiment, bspn, db, aspn, resp]
        """
        inputs = {}
        contexts = []
        cell_list = ['user', 'sentiment', 'bspn', 'n_strategy', 'aspn', 'resp']
    ...
    #         labels = []
    #         batch_zipped = zip(
    #             turn_batch['user'], turn_batch['bspn'], turn_batch['n_strategy'], turn_batch['aspn'], turn_batch['resp'])
    #         # batch_zipped = zip(
    #         #     turn_batch['user'], turn_batch['bspn'], turn_batch['db'], turn_batch['resp'])
    #         #for u, b, db, r in batch_zipped:
    #         for u, b, ns, a, r in batch_zipped:  
    #             context = u+b+ns+a+r
    #             contexts.append(context)
    #             label = u + r
    #             labels.append(label)
    ...
    #         # batch_zipped = zip(pv_batch,
    #         #                    turn_batch['user'], turn_batch['bspn'], turn_batch['db'],  turn_batch['resp'])
    #         #for ur, u, b, db, r in batch_zipped:
    #         for ur, u, b, ns, a, r in batch_zipped:
    #             context = ur + u + b + ns + a + r
    #             contexts.append(context)
    #             label = ur + u + r
    #             labels.append(label)
    #         inputs['contexts'] = contexts
    ...
    #         contexts = []
    #         batch_zipped = zip(
    #             turn_batch['usdx'], turn_batch['bspn'], turn_batch['resp'])
    ...
    #     else:
    #         contexts = []
    #         batch_zipped = zip(pv_batch['pv_usdx'], pv_batch['pv_bspn'], pv_batch['pv_resp'],
    #                            turn_batch['usdx'], turn_batch['bspn'], turn_batch['resp'])
    #         for pu, pb, pr, u, b, r in batch_zipped:
    #             context = pu + pb + pr + u + b + r
    #             contexts.append(context)
    #         inputs['contexts'] = contexts
    #         contexts_np, lengths = utils.padSeqs_gpt(inputs['contexts'], cfg.pad_id)
    #         inputs['contexts_np'] = contexts_np
    ...
    def convert_batch(self, py_batch, py_prev, first_turn=False):
        ...
        #for item in ['user', 'usdx', 'resp', 'bspn', 'bsdx', 'dspn']:
    ...
    def wrap_result_lm(self, result_dict, eos_syntax=None):
        ...
        field = ['dial_id', 'turn_num', 'user', 'sentiment', 'bspn_gen', 'bsdx', 'resp_gen', 'resp', 'aspn_gen', 'aspn',
                     'dspn_gen', 'dspn', 'bspn']
    ...
    def wrap_result(self, result_dict, eos_syntax=None):
        ...
        if cfg.bspn_mode == 'bspn':
            field = ['dial_id', 'turn_num', 'user', 'sentiment', 'bspn_gen', 'bspn', 'resp_gen', 'resp', 'aspn_gen', 'aspn',
                     'dspn_gen', 'dspn']
            # field = ['dial_id', 'turn_num', 'user', 'bspn_gen', 'bspn', 'resp_gen', 'resp',
            #          'dspn_gen', 'dspn', 'pointer, 'sentiment'']
        elif not cfg.enable_dst: # this
            field = ['dial_id', 'turn_num', 'user', 'sentiment', 'bsdx_gen', 'bsdx', 'resp_gen', 'resp', 'aspn_gen', 'aspn',
                     'dspn_gen', 'dspn', 'bspn']
            # field = ['dial_id', 'turn_num', 'user', 'bsdx_gen', 'bsdx', 'resp_gen', 'resp',
            #          'dspn_gen', 'dspn', 'bspn', 'pointer']
        else:
            field = ['dial_id', 'turn_num', 'user', 'sentiment', 'bsdx_gen', 'bsdx', 'resp_gen', 'resp', 'aspn_gen', 'aspn',
                     'dspn_gen', 'dspn', 'bspn_gen', 'bspn']
            # field = ['dial_id', 'turn_num', 'user', 'bsdx_gen', 'bsdx', 'resp_gen', 'resp',
        ...
        with open(cfg.eval_load_path + '/dialogue_record.csv', 'w') as rf:
            ...
            for dial_id in ordered_dial:
                ...
                for turn_no, turn in enumerate(turns):
                    ...
                    sentiment = decode_fn(
                        turn['sentiment'], eos=ontology.eos_tokens['sentiment'])    
                    bspn = decode_fn(
                    ...
                    turn_record = {'user': user, 'sentiment':sentiment, 'bspn': bspn, 'aspn': aspn,
