...
#     with open('ppd_db/ontology.json', 'r') as f: # read ontology in lower, all the domain-slot values
...
#     with open('CLB/clb-processed/bspn_word_collection.json', 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        data_path = 'CLB/data.json'
        archive = zipfile.ZipFile(data_path + '.zip', 'r')
        self.convlab_data = json.loads(archive.open(data_path.split('/')[-1], 'r').read())#.lower())
        self.delex_sg_valdict_path = 'CLB/clb-processed/delex_single_valdict.json'
        self.delex_mt_valdict_path = 'CLB/clb-processed/delex_multi_valdict.json'
        self.ambiguous_val_path = 'CLB/clb-processed/ambiguous_values.json'
        self.delex_refs_path = 'CLB/clb-processed/reference_no.json'
    ...
    def delex_by_annotation(self, dial_turn):
        ...
        for s in span:
            ...
            ####################################################################################
            ##### COMMENT OUT THIS PORTION IF YOU WANT TO GENERATE DELEXICALIZED RESPONSES #####
            ####################################################################################
    ...
    #     text = re.sub(r'\$\d+|\$?\d+.?(\d+)?\s(euros?|EUR?|gbps?)', '[value_price]', text)
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        for fn, raw_dial in tqdm(list(self.convlab_data.items())):
            ...
            for turn_num, dial_turn in enumerate(raw_dial['log']):
                ...
                # print("NEGO STRATEGIES: ",dial_turn['n_strategy'])
                # print(type(dial_turn['n_strategy']))
                ...
                if not dial_state:   # user
                    ...
                    for act in dial_turn['dialog_act']:
                        #print(act)
                        if len(act.split('-'))>=1 and len(act.split('-'))<=3:
                            if len(act.split('-'))==3:   #### MY ADDITION
                                act_l = act.split('-')
                                last_two = '_'.join(act_l[1:])
                                act = '-'.join([act_l[0],last_two])
                    ...
                    sl = re.findall("'([^']*)'",dial_turn['n_strategy'])
                    n_strategies = []
                    for n_strategy in sl:
                        n_strategies.append("["+n_strategy+"]")
                    print("negotiation strategies: ",n_strategies)

                    sentiment = [str(dial_turn['sentiment'])]
        ...
        self.vocab.save_vocab('CLB/clb-processed/vocab')
        with open('CLB/clb-analysis/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open('CLB/clb-analysis/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    # db_paths = {
    #         'Phone': 'ppd_db/phone_db.json',
    #         'Laptop': 'ppd_db/laptop_db.json',
    #         'Camera': 'ppd_db/camera_db.json',
    #         'Tablet': 'ppd_db/phone_db.json',
    #         'Computer':'ppd_db/laptop_db.json'
    #     }
    # get_db_values('ppd_db/value_set.json') # 
    # preprocess_db(db_paths)
    dh = DataPreprocessor()
    data = dh.preprocess_main()
    if not os.path.exists('CLB/clb-processed'):
        os.mkdir('CLB/clb-processed')

    with open('CLB/clb-processed/data_for_damd.json', 'w') as f:
